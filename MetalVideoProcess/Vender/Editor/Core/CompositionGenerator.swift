...
public class CompositionGenerator {
    ...
    public func buildPlayerItem(customVideoCompositorClass: AVVideoCompositing.Type = VideoCompositor.self) -> AVPlayerItem {
        let composition = buildComposition()
        let playerItem = AVPlayerItem(asset: composition)
        self.needRebuildComposition = false
        playerItem.videoComposition = buildVideoComposition(customVideoCompositorClass: customVideoCompositorClass)
    ...
    public func buildImageGenerator() -> AVAssetImageGenerator {
        ...
        imageGenerator.videoComposition = buildVideoComposition(customVideoCompositorClass: VideoCompositor.self)
    ...
    public func buildComposition() -> AVComposition {
        ...
        timeline.videoChannel.enumerated().forEach({ (offset, provider) in
            for index in 0..<provider.numberOfVideoTracks() {
                let trackID: Int32 = getVideoTrackID(for: index) + Int32((offset) * 1000)
                provider.trackID = trackID
        ...
        timeline.audioChannel.enumerated().forEach { (offset, provider) in
            for index in 0..<provider.numberOfAudioTracks() {
                let trackID: Int32 = getAudioTrackID(for: index) + Int32((offset) * 1000)
...
//                    }
        ...
        debugPrint("Composition Info:", composition)
        self.composition = composition
        return composition
    }
    
    public func buildVideoComposition(customVideoCompositorClass: AVVideoCompositing.Type? = VideoCompositor.self) -> AVVideoComposition? {
