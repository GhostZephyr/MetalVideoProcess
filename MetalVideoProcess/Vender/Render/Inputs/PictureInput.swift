...
public class PictureInput: ImageSource {
        ...
        if let texture = internalTexture {
            ...
            if synchronously {
                do {
                    ...
                    self.updateTargetsWithTexture(self.internalTexture!, trackID: self.trackID)
                    self.hasProcessedImage = true
                } catch {
    ...
    public func transmitPreviousImage(to target: ImageConsumer, atIndex: UInt) {
        if hasProcessedImage {
            target.newTextureAvailable(self.internalTexture!, fromSourceIndex: atIndex, trackID: self.trackID)
        }
    }
}
