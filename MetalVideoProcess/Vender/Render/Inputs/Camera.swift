...
public enum PhysicalCameraLocation {
    ...
    func device() -> AVCaptureDevice? {
        let discovery = AVCaptureDevice.DiscoverySession.init(deviceTypes: [.builtInDualCamera,
                                                                            .builtInWideAngleCamera,
                                                                            .builtInTelephotoCamera],
                                                              mediaType: .video,
                                                              position: self.captureDevicePosition())
        for case let device in discovery.devices {
...
public class Camera: NSObject, ImageSource, AVCaptureVideoDataOutputSampleBufferDelegate {
    ...
    public func captureOutput(_ output: AVCaptureOutput, didOutput sampleBuffer: CMSampleBuffer, from connection: AVCaptureConnection) {
        ...
        cameraFrameProcessingQueue.async {
            ...
            if texture != nil {
                self.updateTargetsWithTexture(texture!, trackID: self.trackID)
            }
