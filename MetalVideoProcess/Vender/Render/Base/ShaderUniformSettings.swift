...
public class ShaderUniformSettings {
    public var uniformValues: [Float] = []
    private var buffers: [Float] = []
    private var uniformValueOffsets: [Int] = []
    public var colorUniformsUseAlpha: Bool = true
    let shaderUniformSettingsQueue = DispatchQueue(
        label: "com.wangrenzhu.metalRender.shaderUniformSettings",
        attributes: [])
    public let uniformLookupTable: [String: Int]
    public let uniformLookupTableWithType: [String: (Int, MTLDataType)]
    ...
    public init(uniformLookupTable: [String: (Int, MTLDataType)],
        ...
        self.uniformLookupTableWithType = uniformLookupTable
    ...
    public var iGlobalTime: Float = 0.0
    ...
    public func restoreShaderSettings(renderEncoder: MTLRenderCommandEncoder) {
        shaderUniformSettingsQueue.sync {
            ...
            let progressBuffer = self.currentDevice.device.makeBuffer(bytes: &self.progress, length: MemoryLayout<Float>.size, options: MTLResourceOptions.storageModeShared)!
            
            renderEncoder.setFragmentBuffer(uniformBuffer, offset: 0, index: 1)
            renderEncoder.setFragmentBuffer(iGlobalTimeBuffer, offset: 0, index: 2)
            renderEncoder.setFragmentBuffer(progressBuffer, offset: 0, index: 3)
            //uniform 也传给vertex，方便调用
            renderEncoder.setVertexBuffer(uniformBuffer, offset: 0, index: 30)
        }
    }
    
    public func restoreShaderSettings(computeEncoder: MTLComputeCommandEncoder) {
        shaderUniformSettingsQueue.sync {
            guard (uniformValues.count > 0) else
            {
                
                return
            }
            
            let uniformBuffer = self.currentDevice.device
                .makeBuffer(bytes: uniformValues,
                            length: uniformValues.count * MemoryLayout<Float>.size,
                            options: MTLResourceOptions.storageModeShared)!
            uniformBuffer.label = "computeUniformBuffer"
            ...
            computeEncoder.setBuffer(iGlobalTimeBuffer, offset: 0, index: 2)
            computeEncoder.setBuffer(progressBuffer, offset: 0, index: 3)
            
            guard (buffers.count > 0) else { return }
            //这里上传长度不确定的Buffer数组
            let bufs = self.currentDevice.device
                .makeBuffer(bytes: buffers,
                            length: buffers.count * MemoryLayout<Float>.size,
                            options: MTLResourceOptions.storageModeShared)
            bufs?.label = "computeFloatBuffer"
            computeEncoder.setBuffer(bufs , offset: 0, index: 1)
